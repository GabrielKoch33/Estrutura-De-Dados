...
var duplicada: boolean;
begin
		duplicada:= False;
		while (ref_aux2 <> nil) and (duplicada = False) do
		begin
...
verifica_duplicada_no_dict:= duplicada;
end;
////////////////////////////////////////////////////////////
...
begin
		duplicada:= False;
    while (ref_aux <> nil) and (duplicada = False) do
    	begin
...
begin 
    ...
    begin
        ...
        pc_doadora^.ponteiro_dict := aux3;
    end
    else
        begin
        		node_receptora^.ponteiro_dict := pc_doadora^.ponteiro_dict;
            pc_doadora^.ponteiro_dict := aux3;
	...
	if node = nil then
		...
		begin;
          ...
          ref_descritor.tail_PalavrasChaves		 := node; //fim também, afinal o unico elemento é o primeiro e o ultimo 
			...
			else // possui elementos   // verfica duplicatas quando se tem pelo menos um elemento 
				begin
          ...
          else // caso n possua duplicadas
          begin
	      		while (aux <> nil) and (aux^.palavra_chave < ref_descritor.palavra_chave_user) do
						begin
						    aux := aux^.proximo;  
						end;
						if aux = ref_descritor.head_PalavrasChaves then //insere no inicio 
						begin
							   node^.anterior     												 := nil;
							   node^.palavra_chave												 := ref_descritor.palavra_chave_user;
							   node^.ponteiro_dict												 := nil;
                 ...
                 limpa_tela();
						...
						else if aux = nil then //insere no fim 
						begin
                 ...
                 limpa_tela();	
						...
						else // se n for o ultimo e nem o primeiro, então seta aux2 uma posição antes
						begin
						     aux2																				 := aux^.anterior;
                 ...
                 if node^.ponteiro_dict = nil then
                    ...
                    writeln('Palavra-Chave: "',node^.palavra_chave,'" adicionada e os devidos verbetes foram migrados!');
								 writeln('anterior:', node^.anterior^.palavra_chave,'posterior:',node^.proximo^.palavra_chave); 	 
                 limpa_tela();	
						end;
					end;
				end;
...
begin
		if ref_descritor.head_PalavrasChaves = nil then//não existe nenhum elemento
		begin
						  ...
						  if aux^.ponteiro_dict = nil then // não existem palavras, logo n checa duplicada
							begin  
								...
								begin
									...
									begin // aux 3 e aux 2 estão dentro do dict de aux, verificando a posição de inserção
										aux3 := nil; 
										while (aux2 <> nil) and (aux2^.verbete_PTBR < ref_descritor.palavra_portugues) do
										  	...
										  	node^.verbete_ING 				:= ref_descritor.palavra_ingles;
												...
												if aux3 = nil then
													aux^.ponteiro_dict := node //inseção no inicio do dict
												else
...
begin
   ...
   begin                                                         
      i := 0;
      writeln('=====================================================');	
      writeln('Lista de Palavras-Chaves vazia. Nada para exibir :( '); //OK
   end
   
   else
   begin
    	ler_palavra_chave(ref_descritor);   // User informa PC
			limpa_tela();      
